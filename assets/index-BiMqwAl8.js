(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`/olives_website/assets/bunny_song-BeycIiMq.m4a`,t=`/olives_website/assets/how_bunnies_survive-DIPZCCP-.png`,n=`/olives_website/assets/bunny-meadow-U4bUD9eb.png`,r=[{icon:`EAR`,title:`Super listening ears`,copy:`Big ears help bunnies notice danger early, and those ears also help them stay cool on warm days.`},{icon:`ZIG`,title:`Zoomy zigzags`,copy:`When a bunny is chased, strong back legs can carry it in quick, twisty hops toward a hiding place.`},{icon:`HOME`,title:`Secret hidey-homes`,copy:`Some rabbits live in burrows called warrens, while many cottontails hide in grass, brush, or shallow nests.`},{icon:`NIB`,title:`Nibble smart`,copy:`Bunnies eat grasses and soft plants, often feeding near cover so they can pop back to safety fast.`}];document.querySelector(`#app`).innerHTML=`
  <main class="page-shell">
    <section class="hero" aria-labelledby="site-title">
      <img class="hero-art" src="${n}" alt="Two cheerful bunnies in a sunny meadow beside a burrow and rainbow." />
      ...
      <div class="hero-copy">
        ...
        <h1 id="site-title" aria-label="Olive's Best Website">
          ${`olive's best website`.split(` `).map((e,t)=>`<span class="title-word">${e.split(``).map((e,n)=>`<span style="--letter:${t*8+n}">${e}</span>`).join(``)}</span>`).join(``)}
        </h1>
        <p class="intro">
        ...
        <div class="hero-actions">
          ...
          <div class="music-player" aria-label="Bunny song controls">
            <audio class="bunny-audio" src="${e}" preload="metadata" loop></audio>
    ...
    <section class="survival-poster" aria-labelledby="poster-title">
      ...
      <img src="${t}" alt="A colorful illustrated guide called how bunnies survive." />
    </section>
...
`;var i=document.querySelector(`.sparkle-button`),a=document.querySelector(`.sparkle-field`),o=document.querySelector(`.bunny-audio`),s=document.querySelector(`.play-button`),c=document.querySelector(`.mute-button`),l=document.querySelector(`.volume-slider`);if(i?.addEventListener(`click`,()=>{a?.classList.remove(`burst`),window.requestAnimationFrame(()=>{a?.classList.add(`burst`)})}),o&&s&&c&&l){o.volume=Number(l.value);let e=()=>{s.textContent=o.paused?`Play song`:`Pause song`,s.setAttribute(`aria-label`,o.paused?`Play bunny song`:`Pause bunny song`),c.textContent=o.muted||o.volume===0?`Unmute`:`Mute`,c.setAttribute(`aria-label`,o.muted?`Unmute bunny song`:`Mute bunny song`)};s.addEventListener(`click`,async()=>{o.paused?await o.play():o.pause(),e()}),c.addEventListener(`click`,()=>{o.muted=!o.muted,e()}),l.addEventListener(`input`,()=>{o.volume=Number(l.value),o.muted=o.volume===0,e()}),o.addEventListener(`play`,e),o.addEventListener(`pause`,e),e()}